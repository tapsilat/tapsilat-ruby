RSpec.describe Tapsilat::OrderResponse do
  ...
  describe 'attribute readers' do
    it 'returns correct values for all attributes' do
      ...
      expect(order_response.buyer.to_h).to eq(order_data['buyer'].transform_keys(&:to_sym))
      expect(order_response.shipping_address.to_h).to eq(order_data['shipping_address'].transform_keys(&:to_sym))
      expect(order_response.billing_address.to_h).to eq(order_data['billing_address'].transform_keys(&:to_sym))
      expect(order_response.basket_items.map(&:to_h)).to eq(order_data['basket_items'].map { |i| i.transform_keys(&:to_sym) })
