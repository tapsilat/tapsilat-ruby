  ...
  class Orders
    ...
    def create(order_data)
      with_retry do
        ...
        response = @client.post('/order/create', body: validated_data.to_json)

        # Check if response indicates success or error based on API format
        if response && response['error']
          # Handle API errors returned in response
          error_message = response['error']
          error_code = response['code']
          raise OrderCreationError, "#{error_message} (Code: #{error_code})"
        elsif response && response['order_id']
          # Success - response contains order_id and reference_id
          OrderResponse.new(response)
        else
          # Handle unexpected response format
          raise OrderCreationError, 'Order creation failed - unexpected response format'
        end
    ...
    def get(order_id)
      with_retry do
        ...
        response = @client.get("/order/#{order_id}")

        # Check if response indicates error based on API format
        if response && response['error']
          error_message = response['error']
          error_code = response['code']
          case error_code
          when 101160 # rubocop:disable Style/NumericLiterals
            raise OrderNotFoundError, "#{error_message} (Code: #{error_code})"
          else
            raise OrderAPIError, "#{error_message} (Code: #{error_code})"
          end
        elsif response && response['reference_id']
          # Success - response contains order data
          OrderResponse.new(response)
        else
          # Handle unexpected response format
          raise OrderAPIError, 'Failed to get order - unexpected response format'
        end
      rescue OrderValidationError
        raise # Re-raise validation errors as-is
      rescue Tapsilat::Error => e
    ...
    def get_status(order_id)
      with_retry do
        ...
        response = @client.get("/order/#{order_id}/status")
        ...
        if response && response['error']
          ...
          error_code = response['code']
          case error_code
          when 100810 # rubocop:disable Style/NumericLiterals
            raise OrderNotFoundError, "#{error_message} (Code: #{error_code})"
          else
            raise OrderAPIError, "#{error_message} (Code: #{error_code})"
          end
        elsif response && response['status']
          # Success - response contains status
          OrderStatusResponse.new(response)
        else
          # Handle unexpected response format
          raise OrderAPIError, 'Failed to get order status - unexpected response format'
        end
      rescue OrderValidationError
        raise # Re-raise validation errors as-is
      rescue Tapsilat::Error => e
        ...
        when /Resource not found/
          raise OrderNotFoundError, "Order with ID '#{order_id}' not found"
        when /Unauthorized/
          raise OrderAPIError, "Failed to fetch order status - Invalid API credentials: #{e.message}"
        else
          raise OrderAPIError, "Failed to fetch order status: #{e.message}"
        end
      rescue StandardError => e
        raise OrderError, "Unexpected error while fetching order status: #{e.message}"
      end
    end
    ...
    def list(params = {})
      with_retry do
        ...
        response = @client.get('/order/list', query: query_params)
